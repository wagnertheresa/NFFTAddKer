...
different values dmax and fixed Nfeat=2d/3.
"""
import numpy as np
import pandas as pd
...
for i in range(len(dmax)):
    ...
    model = GridSearch(classifier="NFFTKernelRidge", param_grid=param_grid_nfft, balance=False, norm=None, Nfg=1000, threshold=0.0, Nfeat=Nfeat, dmax=dm, pred_type=pred_type, kernel=kernel)
    ...
    ######################################################################################
    ######################################################################################
    print("\nResults for dmax = ", dm, ", Nfeat = ", Nfeat)
    print("-----------------\n")
    ...
    fg_best[dm,Nfeat] = [best, best_res, best_wind, best_time, best_total_time]
